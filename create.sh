...
function customise_rootfs() {
    ...
    sudo cp "$CURRENT_DIR/etc/shadow" /tmp/${version}-pellcorp/squashfs-root/etc/shadow
}
...
old_image_name="F004_ota_img_V1.2.0.10.img"
board_name="F004"
...
if [ ! -f /tmp/$old_image_name ]; then
    ...
    cp $old_image_name /tmp/$old_image_name
fi
...
cp -v /tmp/$version-pellcorp/${image_name} "${CURRENT_DIR}"